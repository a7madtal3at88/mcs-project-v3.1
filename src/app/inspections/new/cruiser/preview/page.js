...
export default function CruiserPreviewPage() {
  ...
  useEffect(() => {
    const stored = JSON.parse(localStorage.getItem("draft_cruiser_inspection") || "null");
    setDraft(stored);
  }, []);
  ...
  const onSave = () => {
    ...
    saveReport(report);
    localStorage.removeItem("draft_cruiser_inspection");
    router.push("/saved");
  };
  ...
  if (!draft) {
    return (
      <main style={{ padding: 24, fontFamily: "system-ui" }}>
        <h1 style={{ marginTop: 0 }}>Cruiser Preview</h1>
        <p style={{ color: "#666" }}>مفيش بيانات Preview… ارجع للفورم.</p>

        <button onClick={() => router.push("/inspections/new/cruiser")} style={btn}>
          Back to Form
        </button>
      </main>
    );
